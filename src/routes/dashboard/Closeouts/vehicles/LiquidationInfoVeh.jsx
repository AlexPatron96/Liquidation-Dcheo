...
const LiquidationInfoVeh = () => {
    ...
    const printAction = (dataProcess) => {
        ...
        principal.total_delivery_bills = parseFloat(dataProcess?.total_delivery_bills);
        principal.total_collection_bills = parseFloat(dataProcess?.total_collection_bills);
        ...
        arraySendLiq.push(dataProcess?.products_returned?.bill_product_return || []);
        ...
        arraySendLiq.push(`${dataProcess?.vehicle.enrollment} - ${dataProcess?.vehicle.driver}`)
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                sessionStorage.setItem("printVehicle" + dataProcess?.settlement_code, JSON.stringify(arraySendLiq));
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex", gap: "1.5rem", margin: "2rem 0" }}>
                ...
                <div style={{ width: "50%", overflowY: "scroll", height: "500px" }}>
                        ...
                        <tbody>
                            {
                                LiquidionVehView.map((liq, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{liq?.vehicle?.driver} - {liq?.vehicle?.enrollment}</td>
                                        <td>$ {(parseFloat(liq.balance)).toFixed(2)}</td>
                                        <td>
                                            <div style={{ display: "flex", flexDirection: "row", gap: "0.5rem", justifyContent: "center" }}>
                                                <div>
                                                    <Button onClick={() => { printAction(liq) }} variant='info' style={{ width: "40px", padding: "0.25rem" }}>
                                                        <i className="fa-solid fa-print bx-fw"></i>
                                                    </Button>
                                                </div>
                                                <div>
                                                    <Button onClick={() => { clickViewAction(liq) }} variant='success' style={{ width: "40px", padding: "0.25rem" }}>
                            ...
                            <div ref={sectionRef}>
                                ...
                                <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                                    <div>
                                        ...
                                        <h5>Se esta liquidando al Vehiculo: <span style={{ color: "#02B875" }}> {dataView?.vehicle.driver}</span>  </h5>
                                    ...
                                    <div style={{ border: "2px solid grey", width: "225px", height: "150px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
                                        <h5 style={{ fontSize: "50px", color: `${dataView?.balance_gen_veh > 0 ? "#02B875" : dataView?.balance_gen_veh < 0 ? "#C20114" : "#02B875"}` }}>
                                        ...
                                        <div style={{ display: "flex", gap: "1rem", flexDirection: "row", width: "800px" }}>
                                            ...
                                            <div style={{ width: "180px" }}>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    ...
                                                    <span>$ {parseFloat(dataView?.discounts_veh?.total_other).toFixed(2)}</span>
                                                </div>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>T. Productos: </span>
                                                    <span>$ {parseFloat(dataView?.products_returned?.total).toFixed(2)}</span>
                                                </div>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>T. Gastos: </span>
                                                    <span>$ {parseFloat(dataView?.expense_veh?.total).toFixed(2)}</span>
                                                </div>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>T. Dinero: </span>
                                                    <span>$ {parseFloat(dataView?.cash_veh?.total).toFixed(2)}</span>
                                                </div>
                                                ...
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>Total: </span>
                                                    <h5>$ {parseFloat(dataView?.total_received).toFixed(2)}</h5>
                                                </div>
                                            </div>
                                            ...
                                            <div style={{ width: "180px" }}>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>T. Cobrado: </span>
                                                    <span>$ {parseFloat(dataView?.total_collection_bills).toFixed(2)}</span>
                                                </div>
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                ...
                                                <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                    <span>Total:</span>
                                                    <h5>$ {parseFloat(dataView?.total_sent).toFixed(2)}</h5>
                                                </div>
                                            ...
                                            <div style={{ width: "180px", textAlign: "center" }}>
                                                ...
                                                <span style={{ color: `${dataView?.balance > 0 ? "#FFAC42" : dataView?.balance < 0 ? "#C20114" : "#02B875"}`, fontSize: "12px" }}>
                                                    {dataView?.balance > 0 ?
                                    ...
                                    <div style={{ display: "flex", flexDirection: "row", gap: "1.75rem" }}>
                                        ...
                                        <div style={{ width: "250px", height: "300px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                ...
                                                <div style={{ fontSize: "12px" }}>
                                                    {((dataView?.cash_veh?.settlement_code))}
                                                </div>
                                            </div>
                                        ...
                                        <div style={{ width: "250px", height: "300px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                ...
                                                <div style={{ fontSize: "12px" }}>
                                                    {((dataView?.expense_veh?.settlement_code))}
                                                </div>
                                            </div>
                                        ...
                                        <div style={{ width: "250px", height: "300px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                ...
                                                <div style={{ fontSize: "12px" }}>
                                                    {((dataView?.discounts_veh?.settlement_code))}
                                                </div>
                                            </div>
                                        ...
                                        <div style={{ width: "250px", height: "300px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between" }}>
                                                ...
                                                <div style={{ fontSize: "12px" }}>
                                                    {((dataView?.products_returned?.settlement_code))}
                                                </div>
                                            </div>
                                            ...
                                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "14px" }}>{/********************** ojo revisar*/}
                                                <div>
                                                    <i className="fa-brands fa-stack-overflow bx-fw"></i>
                                                    Total:
                                                </div>
                                                <div>
                                                    $ {(parseFloat(dataView?.products_returned?.total).toFixed(2))}
                                                </div>
                                            </div>
