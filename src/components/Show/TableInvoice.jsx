...
const TableInvoice = ({ data, updateInvo, delInvo, createInvo, transaccionPay, onviewpay, setItemSelect }) => {
    ...
    const handleAdd = () => {
        if (formData.total_bill?.length !== 0 && formData.id_seller !== "" && formData.deliver_date !== "" &&
            formData.id_client !== "") {
            delete formData.id;
            ...
            (formData?.total_bill.length === 0 ?
                setError(prevState => ({ ...prevState, total_bill: "Ingrese un Digito" })) : '');

            (formData.id_seller.length === 0 ?
                setError(prevState => ({ ...prevState, id_seller: "Seleccione un Vendedor" })) : "");

            (formData.deliver_date.length === 0 ?
                setError(prevState => ({ ...prevState, deliver_date: "Seleccione una Fecha" })) : "");

            (formData.id_client.length === 0 ?
                setError(prevState => ({ ...prevState, id_client: "Seleccione un Cliente" })) : "");

            (formData.isWhite !== false ?
                "" : (formData.num_bill?.length === 0 ?
                    setError(prevState => ({ ...prevState, num_bill: "Debe de ingresar una Factura" })) : ""));
        }
    };
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                {error?.id_client ? (<p style={{
                                    fontSize: "10px", position: "absolute", padding: "5px", border: "2px solid red",
                                    borderRadius: "5px", backgroundColor: "#fff",
                                    fontFamily: "var(--paragraph-text)"
                                }}>{error?.id_client}</p>) : null}
                            ...
                            <td>
                                <input
                                    ...
                                    onChange={handleChange}
                                />
                                {error?.num_bill ? (<p style={{
                                    fontSize: "10px", position: "absolute", padding: "5px", border: "2px solid red",
                                    borderRadius: "5px", backgroundColor: "#fff",
                                    fontFamily: "var(--paragraph-text)"
                                }}>{error?.num_bill}</p>) : null}
                            </td>
                            ...
                            <td>
                                <select name="isWhite"
                                    ...
                                    <option value={"true"}>N. De Venta</option>
                                </select>
                            </td>
                            ...
                            <td>
                                <input
                                    ...
                                    onChange={handleChange}
                                />
                                {error?.deliver_date ? (<p style={{
                                    fontSize: "10px", position: "absolute", padding: "5px", border: "2px solid red",
                                    borderRadius: "5px", backgroundColor: "#fff",
                                    fontFamily: "var(--paragraph-text)"
                                }}>{error?.deliver_date}</p>) : null}
                            </td>
                            ...
                            <td >
                                ...
                                {error?.total_bill ? (<p style={{
                                    fontSize: "10px", position: "absolute", padding: "5px", border: "2px solid red",
                                    borderRadius: "5px", backgroundColor: "#fff",
                                    fontFamily: "var(--paragraph-text)"
                                }}>{error?.total_bill}</p>) : null}
                            </td>
                            ...
                            <td>
                                <input
                                    style={{ padding: "3px", width: "70px", fontSize: "13px" }}
                                    className="form-control form-control-sm"
                                    type="text"
                                    name="detail"
                            ...
                            <td>
                                <select name="id_seller"
                                    className="form-select h-25"
                                    style={{ padding: "1px", paddingRight: "18px", width: "110px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                                    value={(formData.id_seller)}
                                    onChange={handleChange}  >
                                    <option >Seleccione Vendedor</option>
                                    {
                                        seller?.map((sell, index) => (
                                            <option key={index} value={(parseInt(sell.id))}>{sell.id} - {(sell.name).substring(0, 15)}</option>
                                        ))
                                    }
                                </select>
                                {error?.id_seller ? (<p style={{
                                    fontSize: "10px", position: "absolute", padding: "5px", border: "2px solid red",
                                    borderRadius: "5px", backgroundColor: "#fff",
                                    fontFamily: "var(--paragraph-text)"
                                }}>{error?.id_seller}</p>) : null}
                            </td>
                        ...
                        <tr key={index} style={{ height: "40px" }}>
                            ...
                            {editingIndex === index ?
                                <td>
                                    <select name="id_status"
                                        className="form-select h-25"
                                        style={{ padding: "3px", width: "90px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                            ...
                            {editingIndex === index ?
                                <td>
                                    <input
                                        style={{ padding: "3px", width: "120px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                                        className="form-control form-control-sm"
                                        type="date"
                                        name="deliver_date"
                            ...
                            {editingIndex === index ?
                                ...
                                (<td >
                                    ...
                                    <h5 style={{ fontSize: "15px", cursor: "pointer" }} onClick={() => itemPast({ item, active: true })} >$ {(item.balance)?.toFixed(2)}</h5>
                            ...
                            {editingIndex === index ?
                                (<td>
                                    <select name="id_seller"
                                        ...
                                        onChange={handleInputChange}       >
                                        <option>{item.seller?.code} -  {(item.seller?.name)?.split(" ")[0]}</option>
                                        {
                                            seller.map((sell, index) => (
                                                <option key={index} value={sell?.id}>({sell?.id}) - {sell?.code} {(sell?.name).substring(0, 15)}</option>
                                            ))
                                        }
                                    </select>
                                </td>)
                            ...
                            <td className='tdBtn' style={{ maxWidth: "145px" }}>
                                {editingIndex === index ? (
                                    <>
                                        ...
                                        <button type="button" className="btn btn-warning btn-actions" style={{ alignItems: 'center' }} onClick={() => { setEditingIndex(null); setEditMode(false); }}>
