...
const Cash = ({ codLiq, receptedCash, codeCashLocalStorage, typeLiquidation, codeCheckLocalStorage, checkmoney }) => {
    ...
    useEffect(() => {
        ...
        cashLocalStorage ? setValue("deposits_money", cashLocalStorage.deposits_money) : "";
        cashLocalStorage ? setValue("check_money", cashLocalStorage.check_money) : "";
    ...
    const getThunkBank = async () => {
        ...
        return axios.get(`${URL_BASE}/api/v1/bank/all`, getConfig())
            .then(res => {
    ...
    const handleAdd = () => {
        ...
        if (formChekData.total !== "") {
            ...
            setCheckView(prevState => ([
                ...prevState, formChekDataView
            ]));
                    ...
                    <div className=''>
                        ...
                        <div>
                            ...
                            <div style={{ border: "2px solid var(--first-color)", padding: "1rem", borderRadius: "5px" }}>
                                ...
                                <div>
                                    <div style={{ display: 'flex', flexDirection: "row", flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between" }}>
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                type="text"
                                                name="number_check"
                                                value={formChekData.number_check}
                                                onChange={handleChange}
                                            />
                                        </div>

                                        <div>
                                            <span style={{ fontSize: "12px" }}>Cliente</span>
                                            ...
                                            <ListGroup multiple="" className={activeListSearchCustomer && searchCustomer.length > 1 ? `listClient` : `none`} >
...

                                        {
                                            typeIsSelected ? (
                                                <div>
                                                    <span style={{ fontSize: "12px" }}>Receptador Entre.</span>
                                                    <select name="references"
                                                        ...
                                                        value={formChekData.references}
                                                        onChange={handleChange} >
                                                        <option >Seleccione Recep</option>
                                                        {
                                                            vehicle.map((veh, index) => (
                                                                <option key={index} value={`${veh?.id}-${veh?.driver}`} > {veh?.id} - {veh?.driver}</option>
                                                            ))
                                                        }
                                                    </select>
                                                </div>
...

                                        {
                                            typeIsSelected ? "" : (
                                                <div>
                                                    <span style={{ fontSize: "12px" }}>Receptador Vend.</span>
                                                    <select name="references"
                                                        className="form-select h-25"
                                                        style={{ padding: "5px", width: "150px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                                                        value={formChekData.references}
                                                        ...
                                                        <option >Seleccione Recep</option>
                                                        {
                                                            seller.map((sell, index) => (
                                                                <option key={index} value={`${sell?.id}-${sell?.name}`} > {sell?.id} - {sell?.name}</option>
                                                            ))
                                                        }
                                                    </select>
                                                </div>
                                        ...
                                        <div>
                                            <span style={{ fontSize: "12px" }}>Banco</span>
                                            <select name="id_bank"
                                                className="form-select h-25"
                                                style={{ padding: "5px", width: "150px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                                        ...
                                        <div>
                                            <span style={{ fontSize: "12px" }}>Endosado</span>
                                            <select name="isEndorsed"
                                                className="form-select h-25"
                                                style={{ padding: "5px", width: "90px", backgroundPosition: "right 0.1rem center", fontSize: "13px" }}
                                                value={formChekData.isEndorsed}
                                                onChange={handleChange} >
                                        ...
                                        <div>
                                            <span style={{ fontSize: "12px" }}>A Nombre De</span>
                                            <input
                                                style={{ fontSize: "13px", width: "110px" }}
                                                className="form-control form-control-sm"
                                                type="text"
                                                name="toName"
                                                value={formChekData.toName}
                                                onChange={handleChange}
                                            />
                                        </div>
                                        ...
                                        <div style={{ paddingTop: "1rem", display: "flex", gap: "0.5rem", justifyContent: "center" }}>
                                            ...
                                            <Button variant="info" onClick={handleBankShow}>
